...
// Manual JWT creation for Google service account
...
async function getAccessToken(serviceAccount: any): Promise<string> {
  ...
  try {
    ...
    console.error('Failed to parse OAuth response JSON:', jsonError)
    throw new Error(`Invalid OAuth response JSON: ${jsonError.message}`)
  }
...
serve(async (req) => {
  try {
    console.log('=== Send Reply Notification Function Started ===')
    ...
    let payload: { record: Balasan }
    let newReply: Balasan
    ...
    try {
      ...
      newReply = payload.record
      console.log('New reply data:', JSON.stringify(newReply, null, 2))
    ...
    console.log('Fetching report data for report_id:', newReply.report_id)
    const { data: report, error: reportError } = await supabaseClient
      .from('reports')
      .select('reporter_id')
      .eq('id', newReply.report_id)
      .single()
    ...
    if (!report?.reporter_id) {
      console.log('Anonymous report, no notification sent')
      return new Response("OK - Laporan anonim, notifikasi tidak dikirim.")
    }

    const studentId = report.reporter_id
    console.log('Student ID:', studentId)

    console.log('Fetching FCM tokens for user:', studentId)
    ...
    if (!tokens || tokens.length === 0) {
      ...
      return new Response(`OK - Tidak ada FCM token ditemukan untuk user: ${studentId}`)
    }

    const fcmTokens = tokens.map(item => item.fcm_token)
    console.log('FCM tokens to send to:', fcmTokens)

    // --- LOGIKA BARU UNTUK API V1 ---

    // 1. Dapatkan kredensial dari Environment Variables yang terpisah
    ...
    // 2. Buat "kartu akses sementara" (Access Token) dengan implementasi manual
    console.log('Getting access token...')
    const accessToken = await getAccessToken(credentials);
    console.log('Access token obtained successfully')

    // 3. Kirim notifikasi ke setiap token satu per satu (FCM v1 tidak support multiple tokens)
    ...
    for (const token of fcmTokens) {
      try {
        ...
        if (fcmResponse.ok) {
          ...
          console.log('FCM success for token:', token.substring(0, 20) + '...', responseBody)
          results.push({ token, success: true })
        } else {
          const errorBody = await fcmResponse.text()
          console.error('FCM error for token:', token.substring(0, 20) + '...', errorBody)
          results.push({ token, success: false, error: errorBody })
        }
      } catch (tokenError) {
        console.error('Error sending to token:', token.substring(0, 20) + '...', tokenError)
        results.push({ token, success: false, error: tokenError.message })
      }
    }
    
    const successCount = results.filter(r => r.success).length
    console.log(`Notifikasi berhasil dikirim ke ${successCount}/${fcmTokens.length} token`)
    ...
    console.error("Error dalam Edge Function:", error)
